...
$(function () {
    $('#chart_crowd_time').highcharts({
		...
		tooltip: {
			formatter:function() {
				return '时间：'+this.x+'号'+'<br>拥堵时间：'+this.y+'分钟';
				}
        },
        legend: {
            layout: 'vertical',
...
$(function () {
    $('#chart_car_speed').highcharts({
       chart: {
        ...
        tooltip: {
			formatter:function() {
				return '时间：'+this.x+'号'+'<br>平均车速：'+this.y+'公里/小时';
				}
        },
        legend: {
            layout: 'vertical',
...
function state_data_update(data,zone_name) {
	...
	for (var i = 0;i<chart_month_data.length;i++) {
		...
		add_traf_ctb_data(i,time,day,val)
		}
	$('#chart_traf_flow').highcharts().series[0].setData(data);
	$('#chart_traf_flow').highcharts().setTitle({text:zone_name+'交通状态月变化情况'});
	}
...
function generate_state_data(date) {
	...
	pdate = new Date(date.year,date.month-1,1)
	ldate = new Date(date.year,date.month,1)
