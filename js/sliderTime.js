...
function f_sliderSetValue (n_value, b_noInputCheck) {
	...
	e_input.value = intToTime(n_value);
}
...
function intToTime(num){
	//判断类型
	if(typeof(num)=='number'){//alert(num instanceof Number); 
		if(num > 86399)
			num = parseInt(num % 86399);
		var hourNum = parseInt(num/3600);//parseInt()可以获得整数部分，小数部分丢失；
		var temp = num % 3600;
		var minuteNum = parseInt(temp/60);//JS有内置的方法Math.round()取整数部分（四舍五入）
		var secondNum = temp % 60;
		var hourStr = "";
		//时
		if(hourNum < 10){
			hourStr = '0'+hourNum;
		}else{
			hourStr = hourStr + hourNum;
		}
		//分
		var minuteStr = "";
		if(minuteNum < 10){
		...
		if(secondNum < 10){
			...
			secondStr = secondStr + secondNum;
		}
		return hourStr + ":" + minuteStr + ":" + secondStr;
	}else{
		alert("[ LemonError : the parameter is not a number ]");
		return "";
	}
}
...
function timeToInt(timeStr){
	if(typeof(timeStr)=='string'){
		...
		if(timeArray.length == 3){
			...
			if(hour < 0 || hour > 23){
				alert("[ LemonError : the parameter is not a time ]");
				return 'NaN';
			}
		...
		alert("[ LemonError : the parameter is not a string ]");
		return "";
	}
}
...
function f_sliderMouseUp (e_event, b_watching) {
	if (window.n_activeSliderId != null) {
		...
		//alert('f_sliderMouseUp  o_slider = '+ o_slider );
